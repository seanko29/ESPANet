...
class HFDKDModel(BaseKD):
    ...
    def run_alignment_stage(self, current_iter):
        """Stage 1: Feature Customization - Align teacher features to student space"""
        if self.opt.get('rank', 0) == 0:
            print(f"\r[Iter {current_iter}] ==> STAGE 1: Feature Alignment...", end='')
        ...
        self.net_g.eval()  # Freeze student during alignment
        ...
        with torch.no_grad():
            ...
            # Get student output for reference
            student_output = self.net_g(self.lq)
            # Extract student features
            student_features = self.extract_features(self.net_g, self.lq, is_teacher=False)
        
        # Align teacher features to student feature space
        target_size = student_features.shape[-2:] if len(student_features.shape) == 4 else (64, 64)
        aligned_features = self.feature_aligner(teacher_features, target_size)
        
        # Pass through temporary reconstructor
        temp_output = self.temp_reconstructor(aligned_features)
        
        # Ensure same size as student output
        if temp_output.shape[-2:] != student_output.shape[-2:]:
            temp_output = F.interpolate(
                temp_output, size=student_output.shape[-2:], 
                mode='bicubic', align_corners=False
            )
        
        # Alignment losses
        # l_align = self.l1_loss(temp_output, student_output.detach())
        l_align_feat = self.l1_loss(aligned_features, student_features.detach())
        l_pix_align = self.l1_loss(temp_output, self.gt)
        
        l_total_align = l_align_feat + 0.5 * l_pix_align
        # l_total_align = l_pix_align
        ...
        self.persistent_log_dict['l_align_feat'] = l_align_feat
