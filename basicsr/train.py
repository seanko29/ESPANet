...
def train_pipeline(root_path, override_opt=None):
    ...
    # opt['path']['pretrain_network_t'] = '/home/sean/ACCV/seemoredetails/basicsr/pretrained/001_classicalSR_DIV2K_s48w8_SwinIR-M_x3.pth'
    model = build_model(opt)
    if resume_state:
        # Only load student weights from resume_state
        if 'state_dict' in resume_state and 'params' in resume_state['state_dict']:
            model.net_g.load_state_dict(resume_state['state_dict']['params'], strict=False)
        # If you use EMA:
        if hasattr(model, 'net_g_ema') and 'params_ema' in resume_state['state_dict']:
            model.net_g_ema.load_state_dict(resume_state['state_dict']['params_ema'], strict=False)
        # Restore optimizer/scheduler states (for student/discriminator)
